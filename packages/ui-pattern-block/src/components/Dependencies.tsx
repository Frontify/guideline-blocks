...
export const Dependencies = ({ label, onBlur, placeholder, value, shouldCollapseByDefault }: Props): ReactElement => {
    const [isOpen, setIsOpen] = useState(!shouldCollapseByDefault);
    const [innerValue, setInnerValue] = useState(value);

    return (
        <Accordion label={label} isOpen={isOpen} setIsOpen={setIsOpen}>
            <MantineProvider>
                <JsonInput
                    formatOnBlur
                    autosize
