...
const getThemeColorOrDefault = (
    themeStyle: CSSStyleDeclaration,
    propertyName: string,
    defaultColor: { red: number; green: number; blue: number; alpha: number }
) => {
    const themeColor = themeStyle.getPropertyValue(propertyName);

    if (!themeColor) {
        return defaultColor;
    }

    return rgbStringToRgbObject(themeColor) || defaultColor;
};

export const getDefaultDoColor = (themeStyle: CSSStyleDeclaration) =>
    getThemeColorOrDefault(themeStyle, `${THEME_PREFIX}accent-color-tip-color`, DO_COLOR_DEFAULT_VALUE);

export const getDefaultDontColor = (themeStyle: CSSStyleDeclaration) =>
    getThemeColorOrDefault(themeStyle, `${THEME_PREFIX}accent-color-warning-color`, DONT_COLOR_DEFAULT_VALUE);
