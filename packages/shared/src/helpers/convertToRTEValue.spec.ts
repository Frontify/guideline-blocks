...
describe('String converted to Richtext value', () => {
    test('It should return RTE value with corret textStyle', () => {
        ...
        const result = convertToRTEValue(text, TextStyles.ELEMENT_PARAGRAPH);
        expect(result).toBe('[{"type":"p","children":[{"text":"with text"}]}]');
    });

    test('It should return RTE value with corret textStyle', () => {
        const text = 'Heading 1';
        const result = convertToRTEValue(text, TextStyles.ELEMENT_HEADING1);
    ...
    test('It should return RTE value with corret textStyle and alignment', () => {
        const text = 'text';
        const result = convertToRTEValue(text, TextStyles.ELEMENT_PARAGRAPH, 'center');
