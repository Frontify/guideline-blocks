...
import { BitmapContainerOperator, ImageContainer, ImageElement, ImageStage } from '..';

const IMAGE_CONTAINER_SELECTOR = '[id="image-container"]';
const IMAGE_STAGE_SELECTOR = '[id="image-stage"]';

const BlockContainer = ({ height, padding = 0 }: { height: string; padding?: number }) => {
    ...
    useEffect(() => {
        if (isImageLoaded && stageRef.current && containerRef.current && imageRef.current) {
            ...
            const bitmapContainerOperator = new BitmapContainerOperator(imageContainer, imageStage, imageElement);
...
describe('BitmapContainerOperator', () => {
    it('fits element with 400px height in the center', () => {
        mount(<BlockContainer height="400px" />);
    ...
    it('fits element with 700px height in the center', () => {
        mount(<BlockContainer height="700px" />);
    ...
    it('render element with Padding 0.2 (in percenteges)', () => {
        mount(<BlockContainer height="500px" padding={0.2} />);
