...
it('renders basic code snippet with Github Dark theme', () => {
    const [CodeSnippetWithStubs] = withAppBridgeBlockStubs(CodeSnippetBlock, {
        blockSettings: {
            theme: 'githubDark',
        },
    ...
    mount(<CodeSnippetWithStubs />);
    cy.get(codeSnippetEditorSelector).should('have.css', 'background-color', 'rgb(13, 17, 23)');
});
...
it('renders code snippet without line numbers', () => {
    const [CodeSnippetWithStubs] = withAppBridgeBlockStubs(CodeSnippetBlock, {
        blockSettings: {
    ...
    mount(<CodeSnippetWithStubs />);
    cy.get(codeSnippetLineNumbersSelector).should('not.exist');
});
...
it('renders code snippet with border', () => {
    ...
    mount(<CodeSnippetWithStubs />);
    cy.get(codeSnippetBlockSelector).should('have.css', 'border', '2px solid rgb(22, 181, 181)');
});
...
it('renders code snippet with border radius', () => {
    ...
    mount(<CodeSnippetWithStubs />);
    cy.get(codeSnippetBlockSelector).should('have.css', 'border-radius', '12px');
});
